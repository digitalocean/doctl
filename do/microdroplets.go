...
// MicroDropletSnapshot wraps a godo.MicroDropletSnapshot.
type MicroDropletSnapshot struct {
	*godo.MicroDropletSnapshot
}

// MicroDropletSnapshots is a slice of MicroDropletSnapshot.
type MicroDropletSnapshots []MicroDropletSnapshot
...
type MicroDropletsService interface {
	...
	Update(id string, req *godo.MicroDropletUpdateRequest) (*MicroDroplet, error)
	Delete(id string) error
	ListSnapshots(id string) (MicroDropletSnapshots, error)
}
...
func (s *microDropletsService) Update(id string, req *godo.MicroDropletUpdateRequest) (*MicroDroplet, error) {
	md, _, err := s.client.MicroDroplets.Update(context.TODO(), id, req)
	if err != nil {
		return nil, err
	}
	return &MicroDroplet{MicroDroplet: md}, nil
...
func (s *microDropletsService) ListSnapshots(id string) (MicroDropletSnapshots, error) {
	f := func(opt *godo.ListOptions) ([]any, *godo.Response, error) {
		list, resp, err := s.client.MicroDroplets.ListSnapshots(context.TODO(), id, opt)
		if err != nil {
			return nil, nil, err
		}
	...
	list := make(MicroDropletSnapshots, len(si))
	for i := range si {
		snap := si[i].(godo.MicroDropletSnapshot)
		list[i] = MicroDropletSnapshot{MicroDropletSnapshot: &snap}
	}
	return list, nil
}
