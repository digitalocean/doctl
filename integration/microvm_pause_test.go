...
var _ = suite("compute/microvm/pause", func(t *testing.T, when spec.G, it spec.S) {
	var (
	...
	it.Before(func() {
		...
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			switch req.URL.Path {
			case "/v2/microvms/b2a2f7a4-8d34-4c1c-9c66-3f2b7f8f38f2/pause":
				...
				w.Write([]byte(microVMPauseResponse))
			default:
	...
	when("the id is provided", func() {
		it("pauses the microvm", func() {
			cmd := exec.Command(builtBinaryPath,
				"-t", "some-magic-token",
				"-u", server.URL,
				"compute", "microvm", "pause",
				"b2a2f7a4-8d34-4c1c-9c66-3f2b7f8f38f2",
			)

			output, err := cmd.CombinedOutput()
			expect.NoError(err, fmt.Sprintf("received error output: %s", output))
			expect.Equal(strings.TrimSpace(microVMPauseOutput), strings.TrimSpace(string(output)))
		})
	})
})
