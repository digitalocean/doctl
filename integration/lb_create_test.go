	...
	when("command is create", func() {
		...
		when("the wait flag is passed", func() {
			it("creates a load balancer and polls for status", func() {
				...
				output, err := cmd.CombinedOutput()
				expect.NoError(err, fmt.Sprintf("received error output: %s", output))
				expect.Equal(strings.TrimSpace(lbWaitCreateOutput), normalizeWaitElapsed(strings.TrimSpace(string(output))))
			})
		})
	})
...
Success: Load balancer (4de7ac8b-495b-4884-9a69-1050c6793cd6) is active (elapsed)
Notice: Load balancer created
ID                                      IP    IPv6    Name             Status    Created At              Region    Size        Size Unit    VPC UUID                                Subnet UUID    Tag    Droplet IDs        SSL     Sticky Sessions                                Health Check                                                                                                                                 Forwarding Rules    Firewall Rules                                                            Disable Lets Encrypt DNS Records
	...
	lbCreateResponse = `
{
  "load_balancer": {
	...
	"firewall": {
		"allow": [ "ip:1.2.3.4", "cidr:10.0.0.1/10" ],
		"deny": [ "ip:2.3.4.5", "cidr:10.0.0.2/32" ]
	}
  }
}`
	lbCreateRequest = `
