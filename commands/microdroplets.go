...
func MicroDroplet() *Command {
	cmd := &Command{
		Command: &cobra.Command{
			...
			Long: `The subcommands under ` + "`" + `doctl compute microdroplet` + "`" + ` manage MicroDroplets — lightweight ` +
				...
				`create, inspect, pause, resume, and delete MicroDroplets, and to list their snapshots.`,
		},
	}
	...
	AddStringFlag(cmdMicroDropletCreate, "http-protocol", "", "",
		"HTTP protocol served by the MicroDroplet: `http`, `https`, or `http2`")
	...
	CmdBuilder(cmd, RunMicroDropletSnapshots, "snapshots <microdroplet-id>",
		"List snapshots for a MicroDroplet",
		"Retrieves a list of snapshots for the specified MicroDroplet.",
		Writer, aliasOpt("s"), displayerType(&displayers.MicroDropletSnapshot{}))
...
func RunMicroDropletCreate(c *CmdConfig) error {
	...
	if autoPauseEnabled || autoPauseIdle != "" {
		req.AutoPause = &godo.AutoPauseConfig{IdleTimeout: autoPauseIdle}
		if autoPauseEnabled {
			enabled := true
			req.AutoPause.Enabled = &enabled
		}
	}
...
func RunMicroDropletPause(c *CmdConfig) error {
	return runMicroDropletStateTransition(c, godo.MicroDropletStatePaused)
}

// RunMicroDropletResume transitions a MicroDroplet to the running state.
func RunMicroDropletResume(c *CmdConfig) error {
	return runMicroDropletStateTransition(c, godo.MicroDropletStateRunning)
}

func runMicroDropletStateTransition(c *CmdConfig, state godo.MicroDropletState) error {
	if err := ensureOneArg(c); err != nil {
		return err
	}
	md, err := c.MicroDroplets().Update(c.Args[0], &godo.MicroDropletUpdateRequest{State: state})
	if err != nil {
		return err
	}
	return c.Display(&displayers.MicroDroplet{MicroDroplets: do.MicroDroplets{*md}})
...
// RunMicroDropletSnapshots lists snapshots for a MicroDroplet.
func RunMicroDropletSnapshots(c *CmdConfig) error {
	if err := ensureOneArg(c); err != nil {
		return err
	}
	snaps, err := c.MicroDroplets().ListSnapshots(c.Args[0])
	if err != nil {
		return err
	}
	return c.Display(&displayers.MicroDropletSnapshot{Snapshots: snaps})
}
