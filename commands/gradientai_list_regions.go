...
func ListRegionsCmd() *Command {
	cmd := CmdBuilder(nil, RunGradientAIListRegions, "list-regions", "List inference regions", `The `+"`doctl inference list-regions`"+` command lists all available inference regions.
...
The command returns the following details for each region:
	...
	cmd.Example = `doctl inference list-regions`
