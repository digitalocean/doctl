...
You can use Droplet actions to perform tasks on a Droplet, such as rebooting, resizing, or snapshotting it.`,
	...
	AddActionWaitFlags(cmdDropletActionEnableBackups, false, "Wait for action to complete")
	...
	cmdDropletActionDisableBackups := CmdBuilder(cmd, RunDropletActionDisableBackups,
		...
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionDisableBackups, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
	...
	AddActionWaitFlags(cmdDropletActionChangeBackupPolicy, false, "Wait for action to complete")
	...
	cmdDropletActionReboot := CmdBuilder(cmd, RunDropletActionReboot,
		...
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionReboot, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
	...
	cmdDropletActionPowerCycle := CmdBuilder(cmd, RunDropletActionPowerCycle,
		...
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionPowerCycle, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
Droplets that are powered off are still billable. To stop incurring charges on a Droplet, destroy it.`, Writer,
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionShutdown, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
Droplets that are powered off are still billable. To stop incurring charges on a Droplet, destroy it.`, Writer,
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionPowerOff, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
	...
	cmdDropletActionPowerOn := CmdBuilder(cmd, RunDropletActionPowerOn,
		...
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionPowerOn, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
This also powercycles the Droplet.`, Writer,
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionPasswordReset, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
The Droplet may require additional network configuration to properly use the new IPv6 address. For more information, see: https://docs.digitalocean.com/products/networking/ipv6/how-to/enable`, Writer,
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionEnableIPv6, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
Once you have manually enabled private networking for a Droplet, the Droplet requires additional internal network configuration for it to become accessible through the VPC network. For more information, see: https://docs.digitalocean.com/products/networking/vpc/how-to/enable`, Writer,
		displayerType(&displayers.Action{}))
	AddActionWaitFlags(cmdDropletActionEnablePrivateNetworking, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
	...
	AddActionWaitFlags(cmdDropletActionRestore, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
This command automatically powers off the Droplet before resizing it.`
	...
	AddActionWaitFlags(cmdDropletActionResize, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
To retrieve a list of images on your account, use the `+"`"+`doctl compute image list`+"`"+` command. To retrieve a list of base images, use the `+"`"+`doctl compute image list-distribution`+"`"+` command.`, Writer,
	...
	AddActionWaitFlags(cmdDropletActionRebuild, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
	...
	AddActionWaitFlags(cmdDropletActionRename, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
Use the `+"`"+`doctl compute droplet kernels <droplet-id>`+"`"+` command to retrieve a list of kernels for the Droplet.`, Writer,
	...
	AddActionWaitFlags(cmdDropletActionChangeKernel, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
...
We recommend that you power off the Droplet before taking a snapshot to ensure data consistency.`, Writer,
	...
	AddActionWaitFlags(cmdDropletActionSnapshot, false, "Instruct the terminal to wait for the action to complete before returning access to the user")
