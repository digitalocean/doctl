...
## M1 — Inbound request relay (2–3 days)
...
- **cthulhu + plano stubs**: new `RelayRequest` ControlFrame variant through the
  existing `AttachManager.roundTrip` command/ack machinery. (Note: `proto-public`
  codegen is broken — hand-edit Go/prost stubs as done for v2.)
...
- Merge blockers: upstream the godo vendor patch; fix cthulhu `proto-public`
  toolchain (or document hand-edited stubs as the regen process).
